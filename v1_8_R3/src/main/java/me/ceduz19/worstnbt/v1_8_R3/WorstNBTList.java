...
class WorstNBTList extends AbstractList<NBT> implements NBTList {
    ...
    public NBT get(int index) {
        return WorstNBTInternal.get().toWorst(this.handle.g(index));
    }
    ...
    public NBT remove(int index) {
        return WorstNBTInternal.get().toWorst(this.handle.a(index));
    }
    ...
    public int getInt(int index) {
        NBTBase nbt = this.handle.g(index);
        return nbt instanceof NBTTagInt ? ((NBTTagInt) nbt).d() : 0;
    }
    ...
    public boolean setNBT(int index, @NotNull NBT value) {
        ...
        list.set(index, (NBTBase)value.getHandle());
        return true;
    }
    ...
    public @NotNull NBTType getElementType() {
        return Objects.requireNonNull(NBTType.byId((byte) this.handle.f()));
    }
